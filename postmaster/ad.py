...
class AD(object):
    ...
    def parse_username_input(self, username):
        """ Takes a username and properly formats it for authentication with
        Active Directory
        """
        ...
        if '\\' in username:
            ...
            return '{0}\\{1}'.format(
                self.domain, username_search.group('username'))
        elif self.ldap_connection.authentication != 'NTLM' \
                and re.search('CN=', username, re.IGNORECASE):
            # If the authentication method is not NTLM, then distinguished
            # names are valid usernames
            return username
        else:
    ...
    def get_loggedin_user(self):
        ...
        if self.ldap_connection.bound:
            ...
            if re.search('CN=', self.ldap_connection.user, re.IGNORECASE):
                search_filter = ('(&(objectClass=user)(distinguishedName={0}))'
                                 .format(self.ldap_connection.user))
                user = self.search(search_filter, ['sAMAccountName'])
                return user[0]['attributes']['sAMAccountName']
            # The username is stored as DOMAIN\username, so this gets the
            # sAMAccountName
    ...
    def get_loggedin_user_display_name(self):
        """ Returns the display name or the object name if the display name is
        not available of the logged on user
        """
        user = self.get_ldap_object(
            self.get_loggedin_user(), ['displayName', 'name'])
        # If the displayName is defined, return that, otherwise, return the
        # name which is always defined
    ...
    def get_distinguished_name(self, sam_account_name):
        """ Gets the distinguishedName of an LDAP object based on the
        sAMAccountName
        """
        ldap_object = self.get_ldap_object(sam_account_name)
        return ldap_object['dn']

    def check_nested_group_membership(self, group_sam_account_name,
                                      user_sam_account_name):
        """ Checks the nested group membership of a user by supplying the
        sAMAccountName, and verifies if the user is a part of that supplied
        group. A list with the groups the user is a member of will be returned
        """
        group_dn = self.get_distinguished_name(group_sam_account_name)
        user_dn = self.get_distinguished_name(user_sam_account_name)
        search_filter = \
            '(memberof:1.2.840.113556.1.4.1941:={0})'.format(group_dn)
        # By setting the base to be the user, and the dn searching for as the
        # group, search will return a boolean based on if the user is a member
        # or not
