...
def manage_mock_ldap(f):
    ...
    def wrapped(self, *args, **kwargs):
        # Creates the mocked LDAP instance with the directory defined in the
        # class
        mock_server = ldap3.Server('ldaps://server.domain.local:636',
                                   get_info=ldap3.OFFLINE_AD_2012_R2)
        mock_ldap_connection = ldap3.Connection(
            mock_server, client_strategy=ldap3.MOCK_SYNC,
            authentication=ldap3.SIMPLE)
        ldap_entries = path.join(path.abspath(path.dirname(__file__)),
                                 'ad_ldap_directory.json')
        # Add the mock entries
        mock_ldap_connection.strategy.entries_from_json(ldap_entries)
        # Patch ldap3.Connection so that in ad.__init__, self.ldap_connection
        # is assigned this mocked one
...
class TestAdFunctions:
    ...
    def test_login_pass(self):
        ...
        assert self.ad_obj.login(user_dn, 'P@ssW0rd') is True

    @manage_mock_ldap
    def test_login_fail(self):
        """ Tests the login function with a wrong password and expects a
        return value of ADException
        """
    ...
    def test_parse_username_input_with_domain(self):
        r""" Tests the parse_username_input function when the username input
        is postmaster\username
        """
        assert self.ad_obj.parse_username_input('postmaster\\testUser3') == \
            'postmaster\\testUser3'

    @manage_mock_ldap
    def test_parse_username_input_with_upn(self):
        """ Tests the parse_username_input function when the username input is
        username@postmaster.local
        """
        rv = self.ad_obj.parse_username_input('testUser@postmaster.local')
        assert rv == 'postmaster.local\\testUser'
    ...
    def test_parse_username_input_with_no_domain(self):
        ...
        username
        """
        assert self.ad_obj.parse_username_input('testUser3') == \
            'postmaster.local\\testUser3'

    @manage_mock_ldap
    def test_parse_username_input_with_dn_and_not_ntlm(self):
        """ Tests the parse_username_input function when the username input is
        a distinguishedName and the authentication method is not ntlm
        """
        user_dn = 'CN=Test User,OU=PostMaster,DC=postmaster,DC=local'
        assert self.ad_obj.parse_username_input(user_dn) == user_dn
    ...
    def test_get_loggedin_user_loggedin(self):
        """ Tests the get_loggedin_user function with user being the
        distinguished name and expects the return value to
        be testUser
        """
        user_dn = 'CN=Test User,OU=PostMaster,DC=postmaster,DC=local'
        assert self.ad_obj.login(user_dn, 'P@ssW0rd') is True
    ...
    def test_get_loggedin_user_loggedin_with_dn(self):
        """ Tests the get_loggedin_user function and expects the return value
        to be testUser
        """
        user_dn = 'CN=Test User,OU=PostMaster,DC=postmaster,DC=local'
        assert self.ad_obj.login(user_dn, 'P@ssW0rd') is True
        # For some reason, the mocked instance returns the sAMAccountName as
        # a list
        assert self.ad_obj.get_loggedin_user() == 'testUser'

    @manage_mock_ldap
    def test_get_loggedin_user_display_name(self):
        """ Tests the get_loggedin_user_display_name function which expects
        the return value of Test User
        """
        user_dn = 'CN=Test User,OU=PostMaster,DC=postmaster,DC=local'
        assert self.ad_obj.login(user_dn, 'P@ssW0rd') is True
    ...
    def test_get_loggedin_user_display_name_when_none(self):
        """ Tests the get_loggedin_user_display_name function on a user which
        does not have the displayName attribute specified. It expects the
        return of the name attribute which is testUser2
        """
        user_dn = 'CN=testUser2,OU=PostMaster,DC=postmaster,DC=local'
        assert self.ad_obj.login(user_dn, 'P@ssW0rd') is True
        assert self.ad_obj.get_loggedin_user_display_name() == 'testUser2'

    @manage_mock_ldap
    def test_get_primary_group_dn_of_user(self):
        """ Tests the get_primary_group_dn_of_user function and tests that the
        primary group's distinguished name can be found when passing the
        sAMAccountName of testUser. The distinguished name of Domain Users is
    ...
    def test_check_nested_group_membership(self):
        """ Tests the check_nested_group_membership function and expects the
        return value of the users's distinguished name
        """
        user_dn = 'CN=Test User,OU=PostMaster,DC=postmaster,DC=local'
        assert self.ad_obj.login(user_dn, 'P@ssW0rd') is True
        responses = []
        responses.append([{
            'dn': 'CN=PostMaster Admins,OU=Groups,DC=postmaster,DC=local',
            'attributes': {},
            'raw_attributes': {},
            'type': 'searchResEntry'}])
        responses.append([{
        ...
        # Since ldap3's mock server doesn't support AD specific functions,
        # we need to mock the search
        def mock_search(*args, **kwargs):
            # Set the response because that is what actually returns the
            # results from a search
            self.ad_obj.ldap_connection.response = responses[0]
            # Remove the response just used so the next time this is called,
            # the next response is used
            responses.pop(0)
            return True

        self.ad_obj.ldap_connection.search = mock_search
        assert self.ad_obj.check_nested_group_membership(
            'PostMaster Admins', 'testUser') is True

    @manage_mock_ldap
    def test_check_group_membership_pass_memberof(self):
        """ Tests the check_group_membership function and that the user's group
        membership matches the administrative LDAP group specified in the
        database. A return value of True is expected
        """
        user_dn = 'CN=Test User,OU=PostMaster,DC=postmaster,DC=local'
        assert self.ad_obj.login(user_dn, 'P@ssW0rd') is True
        with patch('postmaster.ad.AD.check_nested_group_membership',
                   return_value=True):
            assert self.ad_obj.check_group_membership() is True

    @manage_mock_ldap
    def test_check_group_membership_pass_primary_group(self):
        """ Tests the check_group_membership function and that the user's
        primaryGroupID matches the administrative LDAP group specified in the
        database. A return value of True is expected
        """
        user_dn = 'CN=testUser2,OU=PostMaster,DC=postmaster,DC=local'
        assert self.ad_obj.login(user_dn, 'P@ssW0rd') is True
    ...
    def test_check_group_membership_fail(self):
        """ Tests the check_group_membership function and that the user is not
        authorized to use PostMaster. A return value of ADException is expected
        """
        user_dn = 'CN=Test User3,OU=PostMaster,DC=postmaster,DC=local'
        assert self.ad_obj.login(user_dn, 'P@ssW0rd') is True
        with patch('postmaster.ad.AD.check_nested_group_membership',
                   return_value=False):
            with pytest.raises(postmaster.ad.ADException) as excinfo:
