...
from postmaster import models, bcrypt
import postmaster.utils
import postmaster.logger
from postmaster.apiv1.utils import is_file_writeable, get_logs_dict
...
class TestUtilsFunctions:
    ...
    def test_maildb_auditing_enabled(self):
        result = postmaster.logger.maildb_auditing_enabled()
        assert isinstance(result, bool)

    def test_login_auditing_enabled(self):
        result = postmaster.logger.login_auditing_enabled()
        assert isinstance(result, bool)
    ...
    def test_get_logs_dict(self, tmpdir):
        ...
        log_file_contents = (
            '{"admin": "admin", "category": "audit", "message": "The alias '
            '\\"rawr@postmaster.com\\" was created successfully", '
            '"timestamp": "2016-07-01T00:41:06.330000Z"}\n{"admin": "admin", '
            '"category": "audit", "message": "The alias '
            '\\"rawr2@postmaster.com\\" was created successfully", '
            '"timestamp": "2016-07-01T00:42:06.330000Z"}\n')
    ...
    def test_json_logger(self, tmpdir):
        log_file = tmpdir.join('postmaster.log')
        app.config['LOG_LOCATION'] = str(log_file)
        postmaster.logger.json_logger(
            'error', 'admin', 'This is a test error message')
    ...
    def test_is_file_writeable_existing_file(self, mock_access):
        """ Tests the is_file_writeable function when a file exists and is
        writable. A return value of True is expected
        """
        assert is_file_writeable('manage.py') is True

    @patch('os.access', return_value=True)
    def test_is_file_writeable_nonexisting_file(self, mock_access):
        """ Tests the is_file_writeable function when a file does not exist
        but the path is writable. A return value of True is expected
        """
        filename = 'S0meNonExistenFil3.SomeExtension'
        assert is_file_writeable(filename) is True

    @patch('os.access', return_value=False)
    def test_is_file_writeable_existing_file_fail(self, mock_access):
        """ Tests the is_file_writeable function when a file exists and is
        not writable. A return value of False is expected
        """
        assert is_file_writeable('manage.py') is False

    def test_is_file_writeable_nonexisting_file_fail(self):
        """ Tests the is_file_writeable function when a file does not exist
        and the path is not writable. A return value of True is expected
        """
        some_path = 'S0meDir/S0meNonExistentFil3.SomeExtension'
        assert is_file_writeable(some_path) is False
    ...
    def test_increment_failed_login_new_user(self):
        ...
        db.session.add(test_admin)
        db.session.commit()
        postmaster.utils.increment_failed_login(test_admin_username)
        new_test_admin = models.Admins.query.filter_by(
            username=test_admin_username).first()
    ...
    def test_increment_failed_login_lock(self):
        account_lockout_threshold = int(
            models.Configs.query.filter_by(
        ...
        td = timedelta(minutes=(account_lockout_duration - 1))
        account_lockout_minus_one_min = datetime.utcnow() + td
        one_min_ago = datetime.utcnow() - timedelta(minutes=1)
    ...
    def test_increment_failed_login_time_elapsed(self):
        ...
        test_admin_username = test_admin.username
        test_admin.failed_attempts = account_lockout_threshold - 1
        td = timedelta(minutes=(reset_account_lockout_counter + 1))
        test_admin.last_failed_date = datetime.utcnow() - td
        db.session.add(test_admin)
        db.session.commit()
        postmaster.utils.increment_failed_login(test_admin_username)
        new_test_admin = models.Admins.query.filter_by(
    ...
    def test_increment_failed_login_user_lockout_disabled(self):
        account_lockout_threshold = models.Configs.query.filter_by(
            setting='Account Lockout Threshold').first()
    ...
    def test_clear_lockout_fields_on_user(self):
        ...
        db.session.add(test_admin)
        db.session.commit()
        postmaster.utils.clear_lockout_fields_on_user(test_admin_username)
        new_test_admin = models.Admins.query.filter_by(
            username=test_admin_username).first()
    ...
    def test_reset_admin_password(self):
        ...
        db.session.add(test_admin)
        db.session.commit()
        postmaster.utils.reset_admin_password('test_admin', 'SomeNewPassword')
        new_test_admin = models.Admins.query.filter_by(
            username='test_admin').first()
        assert bcrypt.check_password_hash(
            new_test_admin.password, 'SomeNewPassword') is True

    def test_add_ldap_user_to_db(self):
        """ Tests the add_ldap_user_to_db function and expects that the
        database entry exist after the function is ran
        """
        postmaster.utils.add_ldap_user_to_db('someUser', 'Some User')
        if models.Admins.query.filter_by(
                username='someUser', name='Some User', source='ldap').first():
            assert True
        else:
